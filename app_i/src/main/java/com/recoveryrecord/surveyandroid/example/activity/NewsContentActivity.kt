...
class NewsContentActivity : AppCompatActivity(), SimpleGestureListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        intent.extras?.let { bundle ->
            ...
            if (myReadingBehavior.triggerBy == READING_BEHAVIOR_TRIGGER_BY_NOTIFICATION) {
                val pushNewsRef = db.collection(PUSH_NEWS_COLLECTION)
        ...
        fetchRemoteOne(
            ...
            try {
                ...
                if (content is ArrayList<*>) {
                    mContent = content.filterIsInstance<String>().map { it.trim() }
    ...
    private fun visibilityCheck(n: Int): Future<Int> {
        return executor.submit(object : Callable<Int> {
...
//                while (!activityEnd) {
    ...
    private fun calculateLayout() {
        val dividedBy = when (myReadingBehavior.fontSize) {
            "1" -> 22
            "0" -> 18
            else -> 25
        }
        ...
        for (i in mContent.indices) {
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    if (isChineseChar(para[iter_char_para]) || isExcept(para[iter_char_para]) || isEmoji(
                            para[iter_char_para]
                        )
                    ) {
                        //is chinese
                    ...
                    } else if ((para[iter_char_para] in 'a'..'z') || (para[iter_char_para] in 'A'..'Z') || isLatin(
                            para[iter_char_para]
                        )
                    ) {
                        var tmp_iter_char_line = iter_char_line + 1
    ...
    private fun initLayout() {
        ...
        val textSize = when (myReadingBehavior.fontSize) {
            "1" -> 20
            "0" -> 17
            else -> 23
        }.toFloat()
...
//        myTextViews[0]?.measure(0, 0)
    ...
    private fun addTextViewToLayout(layout: LinearLayout, textView: TextView) {
        layout.addView(textView)
    }

    private fun addImageViewToLayout(layout: LinearLayout, imgUrl: String?) {
    ...
    override fun onPause() {
        ...
        for (iter in dragObjArrayListArray.indices) {
            if (drag_x_1 == 0f && drag_y_1 == 0f) {
            ...
            } else if (drag_x_1 == dragObjArrayListArray[iter]
                ...
                val my_d1: List<String> = ArrayList(
                    listOf(
                        *dateFormat.format(d1).split(" ".toRegex()).dropLastWhile { it.isEmpty() }
                            .toTypedArray()))
                val my_d2: List<String> = ArrayList(
                    listOf(
                ...
                direction += if (drag_y_1 < drag_y_2) "N" else if (drag_y_1 > drag_y_2) "S" else ""
                direction += if (drag_x_1 < drag_x_2) "E" else if (drag_x_1 > drag_x_2) "W" else ""
                drag_str += "$direction#"
                drag_x_1 = 0f
                drag_y_1 = 0f
                drag_x_2 = 0f
...
//    }
    ...
    override fun onSwipe(direction: Int, flingObj: FlingObj) {
        ...
        flingObj.apply {
            ...
            strFling += "$velocitY_Y/"
            var directionF = ""
            directionF += if (poinT_ONE_Y < poinT_TWO_Y) "N" else if (poinT_ONE_Y > poinT_TWO_Y) "S" else ""
            directionF += if (poinT_ONE_X < poinT_TWO_X) "E" else if (poinT_ONE_X > poinT_TWO_X) "W" else ""
            strFling += "$directionF#"
    ...
    companion object {
        ...
        val defaultTextLayoutParams = LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT,
            LinearLayout.LayoutParams.WRAP_CONTENT
        ).apply {
        ...
        private val ch_except = hashSetOf(
            '，',
            '、',
            '。',
            '．',
